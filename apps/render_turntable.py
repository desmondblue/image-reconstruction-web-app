...
for i, obj_path in enumerate(obj_files):
    ...
    cmd = os.path.join(os.getcwd()+'\\ffmpeg\\bin\\ffmpeg.exe')+ ' -framerate 30 -i ' + obj_root + '/rot_%04d.png -vcodec libx264 -y -pix_fmt yuv420p -refs 16 ' + os.path.join(obj_root, file_name + '.mp4')   
    os.system(cmd)
    cmd = 'del %s/rot_*.png' % obj_root
    os.system(cmd) 